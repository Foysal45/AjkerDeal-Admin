...
class OrderReportViewModel(private val repository: AppRepository): ViewModel() {

    val viewState = MutableLiveData<ViewState>(ViewState.NONE)

    fun fetchEmployeeWorkingReport(date: String): LiveData<List<WorkReportData>> {

        viewState.value = ViewState.ProgressState(true)
        val responseBody = MutableLiveData<List<WorkReportData>>()

        viewModelScope.launch(Dispatchers.IO) {
            val response = repository.getEmployeeWorkReportData(date)
