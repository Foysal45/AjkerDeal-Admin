...
class ContactInfoViewModel(private val repository: AppRepository) : ViewModel() {
    ...
    fun getUserInfo(requestBody: AdUserReqBody): LiveData<List<ADUsersModel>> {
        viewState.value = ViewState.ProgressState(true)
        viewModelScope.launch(Dispatchers.IO) {
            val response = repository.getAdUsersByFilter(requestBody)
