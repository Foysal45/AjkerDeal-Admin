...
class LeaveReportViewModel(private val repository: AppRepository): ViewModel() {

    val viewState = MutableLiveData<ViewState>(ViewState.NONE)

    fun fetchEmployeeLeaveReport(): LiveData<LeaveReportResponse> {

        viewState.value = ViewState.ProgressState(true)
        val responseBody = MutableLiveData<LeaveReportResponse>()

        viewModelScope.launch(Dispatchers.IO) {
            val response = repository.getEmployeeLeaveReport()
