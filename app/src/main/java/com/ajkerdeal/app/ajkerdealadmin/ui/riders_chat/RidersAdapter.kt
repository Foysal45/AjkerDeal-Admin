...
class RidersAdapter: RecyclerView.Adapter<RecyclerView.ViewHolder>() {
    ...
    private var userList: MutableList<RiderModel> = mutableListOf()

    var onItemClicked: ((model: RiderModel) -> Unit)? = null

    var getUnseenCount: ((model: RiderModel, unseenCountTv: TextView, progressBar: ProgressBar) -> Unit)? = null
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        if (holder is RidersAdapter.ViewModel) {

            val model = userList[position]
            val binding = holder.binding

            binding.retentionUserName.text = model.name
            ...
            binding.retentionUserPic.let { view ->
                ...
                Glide.with(view)
                    .asBitmap()
                    .apply(myOptions)
                    .load("https://static.ajkerdeal.com/images/bondhuprofileimage/${model.id}/profileimage.jpg")
                    .into(view)
    ...
    fun initLoad(list: List<RiderModel>){
        userList.clear()
